...
# Command handler for /start - Initiates login process
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        response = requests.get(f"{API_URL}/users/{telegram_uid}")
        if not response.json():
            # if no use then create
            response = requests.post(f"{API_URL}/users", json={"nickname": username, "telegram_uid": telegram_uid})
        ...
        response = requests.post(f"{API_URL}/login", params={"telegram_uid": telegram_uid})
        if response.status_code == 200:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "user_info":
        try:
            response = requests.get(f"{API_URL}/users/{telegram_uid}", headers=headers)
            if response.status_code == 200:
        ...
        try:
            response = requests.post(f"{API_URL}/logout", params={"telegram_uid": telegram_uid})
            if response.status_code == 200:
